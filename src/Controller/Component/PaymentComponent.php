...
class PaymentComponent extends Component {
    ...
    public function paypalRecurring($userId, $cardDetails = []) {
        set_time_limit (180);
        ...
        if (!empty($companyInfo['basic_info']) && !empty($companyInfo['location_info'])) {
            ...
            $paypalDetail = $this->PaymentSettings->find()->where(['is_deleted' => 0, 'is_active' => 1])->first();
            if (!empty($paypalDetail)) {
                ...
                $agreementData['paymentDefinition']['cycle'] = 2;
                $agreementData['paymentDefinition']['amount'] = $cardDetails['amount'];                
                $serverHost = Configure::read('PROTOTYPE') . $_SERVER['HTTP_HOST'];
                $agreementData['merchantPreferences']['returnUrl'] = BASE_URL . '/payments/paypalRecurringResponse/' . base64_encode($cardDetails['amount']) . '/' . base64_encode($userId) . '?flag=success';
                $agreementData['merchantPreferences']['cancelUrl'] = BASE_URL . '/payments/paypalRecurringResponse/' . base64_encode($cardDetails['amount']) . '/' . base64_encode($userId) . '?flag=cancel';
                $agreementData['merchantPreferences']['notifyUrl'] = BASE_URL . '/payments/paypalRecurringResponse/' . base64_encode($cardDetails['amount']) . '/' . base64_encode($userId) . '?flag=notify';
                ;
                ...
                try {
                    ...
                    if ($responseData['status']) {                        
                        ...
                        $subscription['agreement_id'] = $responseData['id'];
                        $subscription['status'] = $responseData['state'];
                        $subscription['plan_id'] = $responseData['plan_id'];
                        $subscription['amount'] = $cardDetails['amount'];
