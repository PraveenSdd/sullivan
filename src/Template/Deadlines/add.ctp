<div class="main-content clearfix">
    ...
    <div class="form-default clearfix">
        <h5><?= $this->Flash->render() ?></h5>
           <?php  echo $this->Form->create('Deadlines', array('url' => array('controller' => 'deadlines', 'action' => 'add'),'id'=>'frmDeadlines',' method'=>'post')); ?>
        <div class="row">
                ...
                <label>Permits <span class="text-danger">*</span></label>
                        ...
                        $html= '<option value="">Please select permit</option>';
                            foreach($userPermitList as $list){
                                $html .= <<<EOD
                                        <option value="{$list['id']}" data-permit-id="{$list['permit_id']}" data-user-permit-id="{$list['id']}"> {$list['permit']['name']} ({$list['user_location']['title']} - {$list['operation']['name']}) </option>  
EOD;
                            }
                        ?>
...
//                                  ));
        ...
        <div class="row">
            <div class="col-sm-6 col-xs-6 custom-pop-select6">
