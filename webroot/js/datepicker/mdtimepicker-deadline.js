...
+function ($) {
	var MDTP_DATA = "mdtimepickerdeadline", HOUR_START_DEG = 120, MIN_START_DEG = 90, END_DEG = 360, HOUR_DEG_INCR = 30, MIN_DEG_INCR = 6,
	...
	var MDTimePicker = function (input, config) {
		...
		this.timepicker = {
			overlay : $('<div class="mdtimepickerdeadline hidden"></div>'),
			wrapper : $('<div class="mdtp__wrapper_deadline"></div>'),
			timeHolder : {
		...
		that.setup(picker).appendTo('.timerdeadline');
	...
	MDTimePicker.prototype = {
		...
		setup : function (picker) {
			...
			switch(that.config.theme) {
				...
				break;
				default:
					wrapper.attr('data-theme', $.fn.mdtimepickerdeadline.defaults.theme);
				break;
			}
		...
		show : function () {
			...
			$('body').attr('mdtimepickerdeadline-display', 'on');
		...
		hide : function () {
			...
			setTimeout(function() {
				...
				$('body').removeAttr('mdtimepickerdeadline-display');
	...
	$.fn.mdtimepickerdeadline = function (config) {
		return $(this).each(function (idx, el) {
			var that = this,
				$that = $(this),
				picker = $(this).data(MDTP_DATA);
				options = $.extend({}, $.fn.mdtimepickerdeadline.defaults, $that.data(), typeof config === 'object' && config);
	...
	$.fn.mdtimepickerdeadline.defaults = {
